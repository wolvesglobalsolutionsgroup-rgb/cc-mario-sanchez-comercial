...
-- Catálogo de unidades visible para el público/comercial
CREATE POLICY "Unidades publicas" ON units FOR SELECT USING (true);

-- Políticas de lectura y escritura para administradores autenticados
CREATE POLICY "Admin total chart_of_accounts" ON chart_of_accounts FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin total units" ON units FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin total tenants" ON tenants FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin total contracts" ON contracts FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin total condo_expenses" ON condo_expenses FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin total invoices" ON invoices FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin total payments" ON payments FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin total transactions" ON transactions FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin total alerts" ON alerts FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin total audit_logs" ON audit_logs FOR ALL USING (auth.role() = 'authenticated');
