...
function renderList(items) {
  ...
  items.forEach(item => {
    ...
    const imgSrc = item.image || getSpaceThumb(item);

    card.innerHTML = `
      <img src="${imgSrc}" alt="Vista previa de ${item.name}" loading="lazy" width="64" height="64" class="w-16 h-16 rounded-lg object-cover flex-shrink-0 group-hover:scale-105 transition-transform bg-slate-900 border border-white/10">
